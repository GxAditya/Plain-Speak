...
This migration adds comprehensive free tier support with:
1. User tier management (free/pro/enterprise)
2. Secure Gemini API key storage with encryption
3. Daily upload limits for free tier users
4. Tier-based access controls

## New Features
- Bring-your-own-key functionality for free tier
- 3 uploads per day limit for free users
- Encrypted API key storage
- Daily reset functionality
- Tier information management
*/
...
BEGIN
  ...
  -- Add last_upload_date column (fixed table name reference)
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
...
-- Create a master key for encryption if it doesn't exist
DO $$
DECLARE
  key_count integer;
BEGIN
  SELECT COUNT(*) INTO key_count FROM pgsodium.key;
  IF key_count = 0 THEN
    PERFORM pgsodium.create_key('master_key', 'aead-det');
  END IF;
END $$;
...
BEGIN
  ...
  FROM pgsodium.key 
  WHERE name = 'master_key' 
  LIMIT 1;
  
  IF master_key_id IS NULL THEN
    -- Create master key if it doesn't exist
    SELECT pgsodium.create_key('master_key', 'aead-det') INTO master_key_id;
  END IF;
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE LOG 'Error retrieving Gemini key for user %: %', user_id, SQLERRM;
    RETURN NULL;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- Create new RLS policy for tier-based updates (fixed syntax)
CREATE POLICY "Users can update own profile with tier restrictions"
  ON profiles
  FOR UPDATE
  TO authenticated
  USING (auth.uid() = id)
  WITH CHECK (
    auth.uid() = id AND 
    (
      -- Regular users cannot change their tier
      user_tier = (SELECT user_tier FROM profiles WHERE id = auth.uid()) OR
      -- Admins can update any tier
      EXISTS (
